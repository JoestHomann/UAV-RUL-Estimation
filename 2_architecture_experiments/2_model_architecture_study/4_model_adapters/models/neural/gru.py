"""Implement the causal unidirectional GRU architecture."""
...
class GRURegressor(nn.Module):
    """Encode variable-length histories with a packed unidirectional GRU."""
    ...
    def _right_padded(
        ...
        if torch.any(lengths <= 0):
            raise ModelAdapterError("GRU received a completely padded history")
...
class GRUAdapter(SequenceNeuralAdapter):
    """Fit a packed unidirectional GRU to scaled telemetry windows."""
    ...
    def _build_network(self, training_inputs: NeuralInputs) -> nn.Module:
        ...
        if self.hyperparameters["direction"] != "unidirectional":
            raise ModelAdapterError("Only the causal unidirectional GRU is supported")
