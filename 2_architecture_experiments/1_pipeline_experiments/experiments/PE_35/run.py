"""Matched, resumable ablation of eight channels using the v13 main XGBoost."""
...
def load_source(path):
    spec = importlib.util.spec_from_file_location("pe35_v13", path)
...
def prepare(config):
    ...
    arms = {"baseline": []} | {f"drop_{c.removeprefix('telemetry_')}": [c] for c in config["channels"]} | {"drop_all_eight": config["channels"]}
...
def report(output, config, catalogs):
    ...
    for arm in catalogs:
        ...
        # Bonferroni intervals for the nine predeclared baseline comparisons.
        adjusted_lo, adjusted_hi = np.quantile(changes, [0.025 / 9, 1 - 0.025 / 9])
    ...
    text = ["# PE_35: v13 XGBoost channel ablation", "", "Completed 50 matched fits: baseline, eight individual removals, and removal of all eight.", "",
    ...
    text += ["", "| Removal | RMSE change | Fold wins | Paired UAV 95% interval | Nine-comparison adjusted interval | Assessment |",
             "| --- | ---: | ---: | --- | --- | --- |"]
    ...
    for fold, fitting, stopping, held in folds:
        ...
        for arm, columns in catalogs.items():
            ...
            if cell_json.exists():
                ...
                completed += 1
                print(f"Reused {completed}/50 {arm} fold {fold}", flush=True)
                continue
            indices = [full_columns.index(column) for column in columns]
            print(f"Fitting {completed + 1}/50 {arm} fold {fold}: {len(columns)} features", flush=True)
            ...
            completed += 1
            print(f"Completed {completed}/50 in {elapsed:.1f}s; iteration={model.best_iteration}; RMSE={metrics(selected_endpoints)['rmse']:.4f}", flush=True)
    report(output, config, catalogs)
