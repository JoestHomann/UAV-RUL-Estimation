"""Plot completed PE_39 paired effects without fitting or selecting a model."""
...
SPEC = importlib.util.spec_from_file_location("pe39_runner", HERE / "run.py")
...
def main():
    ...
    order = [arm for arm in runner.build_arms(config["channels"]) if arm != "baseline"]
    effects = effects.loc[order]
    labels = {runner.arm_name([channel]): channel for channel in config["channels"]}
    labels[runner.arm_name(config["channels"])] = " + ".join(config["channels"])
    ...
    fig.subplots_adjust(left=0.28, right=0.985, top=0.985, bottom=0.21)
    ...
    ax.set_ylabel("Channels removed", labelpad=12)
    ...
    fig.savefig(output / "paired_removal_effects.png", dpi=240, facecolor="white",
                bbox_inches="tight", pad_inches=0.025)
    fig.savefig(output / "paired_removal_effects.svg", facecolor="white",
                bbox_inches="tight", pad_inches=0.025)
    plt.close(fig)
    report = output / "report.md"
    marker = "![Paired channel-removal effects](paired_removal_effects.png)"
    content = report.read_text(encoding="utf-8")
    if marker not in content:
        content += "\n" + marker + "\n"
    report.write_text(content, encoding="utf-8")
    print(output / "paired_removal_effects.png")
